...
def sparsegpt(input_tensor, weight_tensor, sparsity_ratio, prune_n, blocksize=128, percdamp=.01, device = "cuda:0", A=False, dtype = torch.float32, nsamples = nsamples):
    ...
    H = torch.zeros(weight_tensor.shape[1], weight_tensor.shape[1]).to(device)
...
# for i in range(3):
    with torch.no_grad():
        ...
        if prune_in_proj == True:
            model_weights[mixer_in_proj_weights[i]] = sparsegpt(input_tensor=rearrange(normalized_hidden_input,"b l d -> d (b l)"), weight_tensor= model_weights[mixer_in_proj_weights[i]],sparsity_ratio=sparsity_ratio,prune_n= prune_n, device=device, dtype = dtype)
        # print(model_weights[mixer_in_proj_weights[i]].shape)
        # print(model_weights[mixer_in_proj_weights[i]])
        ...
        xz = rearrange(
            model_weights[mixer_in_proj_weights[i]] @ rearrange(normalized_hidden_input, "b l d -> d (b l)"),
            "d (b l) -> b d l",
            l=seqlen,
        )
        ...
        if prune_conv1d == True:
            model_weights[mixer_conv1d_weights[i]] = sparsegpt(input_tensor=rearrange(x, "b l d -> (b l) d").T, weight_tensor=model_weights[mixer_conv1d_weights[i]].T, sparsity_ratio=sparsity_ratio, prune_n=prune_n, device=device, dtype = dtype)
        ...
        if prune_x_proj == True:
            model_weights[mixer_x_proj_weights[i]] = sparsegpt(input_tensor=rearrange(x, "b l d -> (b l) d").T, weight_tensor=model_weights[mixer_x_proj_weights[i]], sparsity_ratio=sparsity_ratio, prune_n=prune_n, device=device, dtype = dtype)
        ...
        if prune_dt_proj == True:
            model_weights[mixer_dt_proj_weights[i]] = sparsegpt(input_tensor=rearrange(dt, "b l d -> (b l) d").T, weight_tensor=model_weights[mixer_dt_proj_weights[i]], sparsity_ratio=sparsity_ratio, prune_n=prune_n, device=device, dtype = dtype)
        ...
        if prune_A_log == True:
            ...
            A = sparsegpt(input_tensor=rearrange(dt_act, "b l d -> (b l) d").T, weight_tensor=A.T, sparsity_ratio=sparsity_ratio, prune_n=prune_n, device=device, A=True, dtype = dtype)
            A = A.T
            positive_mask = A > 0
        ...
        if prune_out_proj == True:
            model_weights[mixer_out_proj_weights[i]] = sparsegpt(input_tensor=rearrange(y,"b l d -> (b l) d").T, weight_tensor=model_weights[mixer_out_proj_weights[i]], sparsity_ratio=sparsity_ratio, prune_n=prune_n,device=device, dtype = dtype)
        print(model_weights[mixer_out_proj_weights[i]].shape)
        print(model_weights[mixer_out_proj_weights[i]])
